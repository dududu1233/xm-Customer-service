...
# 角色：知识轨道的编排处理者。
# 功能：KnowledgeHandler 根据知识意图查到对应 provider，调用其检索得到知识片段，再交给 KnowledgeResponder 润色返回。
# 入口：被 engines 层在命中 knowledge 轨道时调用。
# 出口：atguigu.domain.messages、atguigu.domain.state、atguigu.knowledge.intents、atguigu.knowledge.provider.register、atguigu.knowledge.resonder。
