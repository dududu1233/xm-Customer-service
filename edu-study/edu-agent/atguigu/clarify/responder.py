# === 文件：atguigu/clarify/responder.py ===
# 角色：澄清轨道的执行者，按校验失败原因生成追问话术。
# 功能：ClarifyResponder 根据 ClarifyReason 生成对应的澄清/追问文本（含固定话术与 LLM 润色两种路径）。
# 入口：被 engines/validator 或对话引擎在计划校验不通过时调用。
# 出口：atguigu.domain.messages、atguigu.domain.state、atguigu.plan.turn_plan、atguigu.chat_history.builder、atguigu.prompt.loader、atguigu.infrastructure.llm_client。
import json
