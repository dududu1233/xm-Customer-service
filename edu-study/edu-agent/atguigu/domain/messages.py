# === 文件：atguigu/domain/messages.py ===
# 角色：domain 层纯数据模型，定义消息与焦点对象的结构。
# 功能：定义 MessageType、FocusedObject、UserMessage、BotMessage、ProcessedResult、ChatHistoryMessage 等消息相关数据结构及字典互转方法。
# 入口：被几乎所有层（api、services、engines、各 handler/responder、repository）用作消息载体。
