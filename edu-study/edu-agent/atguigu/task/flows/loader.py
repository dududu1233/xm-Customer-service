# === 文件：atguigu/task/flows/loader.py ===
# 角色：task 流程的加载器，把 YAML 配置解析为内存流程对象。
# 功能：FlowLoader 读取 system_flows.yml/user_flows.yml，解析出 FlowList（含 FlowSlot、Flow、CollectionFlowStep 所缺槽位）。
# 入口：被 engines/builder 调用以加载流程。
# 出口：atguigu.task.flows.flows、atguigu.task.flows.steps。
