# === 文件：atguigu/task/flows/executor.py ===
# 角色：task 流程推进器，按 YAML 流程定义驱动业务/系统流程前进。
# 功能：FlowExecutor.execute_flow 双层循环推进步骤（start/end/action/collect），遇到 action 步骤交给 ActionRunner 执行并回写槽位，遇到 collect 步骤处理槽位收集与校验，最终在遇到 action_listen 停下。
# 入口：被 task/handler 调用。
# 出口：atguigu.domain.contexts、atguigu.domain.messages、atguigu.domain.state、atguigu.task.action.runner、atguigu.task.flows.flows、atguigu.task.flows.links、atguigu.task.flows.steps。
