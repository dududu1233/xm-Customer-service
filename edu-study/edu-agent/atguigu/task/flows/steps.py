# === 文件：atguigu/task/flows/steps.py ===
# 角色：task 流程步骤的数据模型，定义步骤类型与解析。
# 功能：定义 FlowStepType 枚举、ResponseDefinition、Validated 及 FlowStep/StartFlowStep/EndFlowStep/ActionFlowStep/CollectionFlowStep，含 from_dict 按类型反序列化与边解析；提供类型→类映射。
# 入口：被 flows/links、flows/flows、flows/loader、flows/executor 引用。
# 出口：atguigu.task.flows.links。
"""
定义步骤
"""
