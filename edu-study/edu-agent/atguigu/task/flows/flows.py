# === 文件：atguigu/task/flows/flows.py ===
# 角色：task 流程的数据模型，描述 YAML 流程与槽位结构。
# 功能：定义 FlowSlot、Flow（含步骤与槽位、get_step_by_id）、FlowList（多流程集合、get_flow_by_id）。
# 入口：被 flows/loader、flows/executor、plan/planner、plan/validator、task/handler、task/commands/processor 等广泛引用。
# 出口：atguigu.task.flows.steps。
