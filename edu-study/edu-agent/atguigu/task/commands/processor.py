# === 文件：atguigu/task/commands/processor.py ===
# 角色：task 命令的执行者，按命令修改对话状态。
# 功能：CommandProcessor.process_command 分发处理四类命令，分别开启/中断/恢复/取消业务流程并激活相应系统流程上下文。
# 入口：被 task/handler 调用。
# 出口：atguigu.domain.contexts、atguigu.domain.state、atguigu.task.commands.command、atguigu.task.flows.flows。
