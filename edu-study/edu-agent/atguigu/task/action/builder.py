# === 文件：atguigu/task/action/builder.py ===
# 角色：task action 的装配器，构建并注册所有 action 到 runner。
# 功能：build_action_runner() 创建 ActionRunner，注册内置 action（response/listen）并自动发现 customer 包下所有 Action 子类完成注册。
# 入口：被 engines/builder 调用以构造 action_runner 注入 TaskHandler。
# 出口：atguigu.task.action.base、atguigu.task.action.builtin.*、atguigu.task.action.register、atguigu.task.action.runner。
import importlib
import inspect
import pkgutil
