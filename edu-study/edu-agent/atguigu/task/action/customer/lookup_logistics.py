# === 文件：atguigu/task/action/customer/lookup_logistics.py ===
# 角色：自定义 action，查询订单物流信息。
# 功能：ActionLookupLogisticsStatus 通过 shared.fetch_logistics 调中台，把物流单号/公司/状态写入 updated_slots 返回。
# 入口：被 action/builder 自动发现并注册到 runner，运行时由 executor 触发。
# 出口：atguigu.domain.state、atguigu.task.action.base、atguigu.task.action.customer.shared。
