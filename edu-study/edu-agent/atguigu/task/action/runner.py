# === 文件：atguigu/task/action/runner.py ===
# 角色：task action 的执行器。
# 功能：ActionRunner.run 按 ActionCall 中的 action 名从注册中心取出 Action 并调用其 run，返回 ActionResult；定义 ActionCall 数据结构。
# 入口：被 task/flows/executor 在推进到 action 步骤时调用。
# 出口：atguigu.domain.state、atguigu.task.action.base、atguigu.task.action.register。
"""
