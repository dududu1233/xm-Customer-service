# === 文件：atguigu/chat_history/builder.py ===
# 角色：对话历史层，负责把内存中的轮次(Turn)对象序列化成供 LLM 使用的历史文本。
# 功能：提供 ChatHistoryBuilder，将用户/机器人消息（文本或卡片对象）拼成 "USER:/BOT:" 格式字符串，并构造 ChatHistoryMessage。
# 入口：被 chitchat/knowledge/clarify 的 responder 及 task action response 等需要拼接历史的模块调用。
