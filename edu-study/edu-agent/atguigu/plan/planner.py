# === 文件：atguigu/plan/planner.py ===
# 角色：plan 层单轮对话的规划器，负责路由分析。
# 功能：TurnPlanner.predict 汇总用户输入/历史/焦点对象/流程/知识意图等，调用 LLM 产出 TurnPlan（含 task/knowledge/chitchat 轨道及命令/意图）。
# 入口：被 engines 层（对话引擎）在每轮对话开始时调用。
# 出口：atguigu.domain.contexts、atguigu.domain.state、atguigu.knowledge.intents、atguigu.plan.turn_plan、atguigu.prompt.loader、atguigu.infrastructure.llm_client、atguigu.chat_history.builder、atguigu.task.flows.flows。
import json
...
class TurnPlanner:
    ...
    def _build_prompt_inputs(self,
        ...
        active_system_task_json = json.dumps(
            state.active_system_task.to_dict(),
            ensure_ascii=False) if state.active_system_task is not None else "null"
        ...
        return {
            ...
            "active_system_task_json": active_system_task_json,
