# === 文件：atguigu/plan/validator.py ===
# 角色：plan 层对单轮计划的校验器。
# 功能：TurnPlanValidator.valid 校验计划命中轨道数量与各轨道内部合法性（task 命令合法性/流程存在性、knowledge 意图与焦点对象是否匹配），返回带 ClarifyReason 的校验结果。
# 入口：被 engines 层在规划后调用。
# 出口：atguigu.domain.state、atguigu.knowledge.intents、atguigu.plan.turn_plan、atguigu.task.flows.flows、atguigu.task.commands.command。
