...
def list_order_items(
    ...
    rows = fetch_all(
        """
        SELECT
            id, order_id, cohort_id, item_name,
            unit_price, discount_amount, payable_amount,
            order_item_status, created_at
        FROM order_item
        WHERE order_id = %s
        ORDER BY id ASC
        """,
    ...
    return ok({"orderId": order_id, "items": items, "total": len(items)})
